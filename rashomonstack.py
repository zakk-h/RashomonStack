...
class StackedRashomon:
    ...
    def _validate_tree(self, tree, X, i):
        ...
        try:
            ...
            if len(pred) != len(X):
                if self.verbose:
                    print(f"Tree {i} returned predictions of length {len(pred)}, "
                        f"but expected {len(X)}.")
                return False
...
X = pd.DataFrame(data_reg.data, columns=data_reg.feature_names)
y = pd.Series(data_reg.target, name="target")

# Get the first 25% of the dataset
num_rows = int(0.1 * len(X))  # Calculate 25% of the dataset
X_reg = X.iloc[:num_rows]
y_reg = y.iloc[:num_rows]
